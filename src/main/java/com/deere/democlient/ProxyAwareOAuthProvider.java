...
public class ProxyAwareOAuthProvider extends DefaultOAuthProvider {
    public ProxyAwareOAuthProvider(final Map<String, Link> links) {
        super(links.get("oauthRequestToken").getUri(),
             ...
             cleanAuthorizationUriForSignPost(links.get("oauthAuthorizeRequestToken").getUri()));
    ...
    private Boolean isProxySet() {
        return Boolean.valueOf(System.getProperty("http.proxySet", "false"));
    }

    private Proxy proxyFromSystemProperties() {
        final String proxyHost = getProperty("http.proxyHost", "your proxy server");
        final Integer proxyPort = valueOf(getProperty("http.proxyPort", "80"));
